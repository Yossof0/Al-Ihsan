...
export default function ThikrCard({ item, displayMode = 'rect', showRewardAlways = true, onIncrement, onReset }) {
  const [tapped, setTapped] = useState(0); // local progress, resets per "set"
  const [showProof, setShowProof] = useState(false);
  const [shareCopied, setShareCopied] = useState(false);

  const target = item.count || 1;
  const progress = Math.min(tapped, target);
  const done = progress >= target;
  const pct = progress / target;

  const handleTap = () => {
    if (done) {
      setTapped(0); // tapping a finished counter resets it for another round
      onReset?.(item);
      return;
    }
    const next = tapped + 1;
    setTapped(next);
    onIncrement?.(item, next);
  };
  ...
  return (
    <div className="rounded-2xl p-4 bg-white dark:bg-layl-900/60 border border-sakeenah-200 dark:border-layl-800 flex items-center gap-4 animate-fade-in">
      ...
      {showProof && (
        <div className="absolute mt-2 max-w-xs p-3 rounded-xl bg-sakeenah-800 dark:bg-layl-700 text-white text-xs shadow-lg z-10 animate-fade-in">
